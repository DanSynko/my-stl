...
public:
    unordered_map() : hm_capacity(3), hm_size(0) {
        buckets = new Bucket * [hm_capacity]();
    }
    unordered_map(std::initializer_list<value_type> hm) : hm_capacity(3), hm_size(0) {
        buckets = new Bucket * [hm_capacity]();
...
int main()
{
    ...
    unordered_map<std::string, std::string> hashmap = {
        {"Car", "BMW"},
        {"Word", "C++"},
        {"Rac", "Volkswagen"}
    };
